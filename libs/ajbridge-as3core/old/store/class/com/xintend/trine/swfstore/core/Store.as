package com.xintend.trine.swfstore.core {
	...
	public class Store extends EventDispatcher implements IStore{
		...
		public static const CLASS_NAME: String = "Store";
		...
		public function init(stage: Stage): void {
			...
			if (stage.loaderInfo.parameters["useCompression"]) {
				_useCompression = stage.loaderInfo.parameters["useCompression"] == "true" || stage.loaderInfo.parameters["useCompression"] == true ;
			}else {
		...
		public function setItem(key:String,data:*): void {
			...
			storeEvent = new StoreEvent(StoreEvent.STORAGE, info,key,  oldValue, data);
		...
		public function removeItem(key: String): void {
			...
			storeEvent = new StoreEvent(StoreEvent.STORAGE,info, key, oldValue);
		...
		public function clear(): void {
			...
			dispatchEvent(new StoreEvent(StoreEvent.CLEAR,CLASS_NAME));
		}
		/**
		 * 获得已存字节大小  单位 B
		 * @return
		...
		public function setMinDiskSpace(value: int): String {
			...
			if (hasAdequateDimensions()) {
				...
				dispatchEvent(new StoreEvent(StoreEvent.ERROR,"Make sure that your application window size is at least 215 x 138 pixels"));
			}
			return status;
		}
		/**
		...
		public function displaySettings(): void {
			if (hasAdequateDimensions()) {
				...
				dispatchEvent(new StoreEvent(StoreEvent.SHOW_SETTINGS,CLASS_NAME));
			}else {
				dispatchEvent(new StoreEvent(StoreEvent.ERROR, "Make sure that your application window size is at least 215 x 138 pixels"));
			}
		}
		...
		protected function save(archive:Object): Boolean {
			...
			try{
				...
				dispatchEvent(new StoreEvent(StoreEvent.ERROR,"SharedObject flush error"));
			}
			switch(result) {
				...
				case SharedObjectFlushStatus.PENDING:
					dispatchEvent(new StoreEvent(StoreEvent.PENDING,CLASS_NAME,archive.hash[archive.hash.length-1]));
					return false;
				break;
				default:
					dispatchEvent(new StoreEvent(StoreEvent.ERROR, "Flash Player Could not write SharedObject to disk."));
					return false;
			}
		...
		protected function readArchive(): Object {
			...
			if(!so.data.hasOwnProperty("archive")){
				...
				dispatchEvent(new StoreEvent(StoreEvent.NEW,CLASS_NAME));
 			}else {
				////	判断是否字节流
				////	是则是经过压缩的数据
				if (so.data.archive is ByteArray) {
					...
					try	{
 						...
 						dispatchEvent(new StoreEvent(StoreEvent.ERROR,"ByteArray uncompress error"));
 					}
